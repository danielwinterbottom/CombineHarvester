...
def PositionedLegend(width, height, pos, offset,yoffset):
    ...
    if pos == 1:
        return R.TLegend(l + o, 1 - t - o - h-yoffset, l + o + w, 1 - t - o-yoffset, '', 'NBNDC')
    if pos == 2:
        c = l + 0.5 * (1 - l - r)
...
#for c in cmb loosemjj_lowboost loosemjj_boosted tightmjj_lowboost tightmjj_boosted lowboost boosted loosemjj tightmjj; do
    ...
    data.GetXaxis().SetTitle('#Delta#phi_{jj}') # radians
    data.GetYaxis().SetTitleSize(0.05)
    ...
    sm.SetLineWidth(3)
    sm.SetLineColor(R.kRed)
    sm.SetMarkerSize(0)
    sm.SetFillStyle(0)

    ps.SetLineWidth(3)
    ps.SetLineColor(R.kBlue)
    ps.SetMarkerSize(0)
    ps.SetFillStyle(0)
