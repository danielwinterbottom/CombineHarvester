...
void AddMSSMRun2Systematics(CombineHarvester & cb, int control_region, bool zmm_fit, bool ttbar_fit) {
  ...
  std::vector<std::string> bbH = {"bbH", "bbH_Htautau", "bbA_Atautau", "bbh_htautau", "bbH-NLO"};
...
/*  cb.cp().process({"TTH_SM125"}).AddSyst(cb, "pdf_Higgs_ttH","lnN",
  ...
  //MSSM theory uncertainties (NLO only)
  // At the moment i have added uncertainties for 3 proposals fro b-tag categories: n_bjets>=1, n_bjets=1, and n_bjets>=1 && n_jets<=1 - comments out as nessesary 

  //cb.cp().process({bbH}).AddSyst(cb, “syst_name”,”lnN”, SystMap::init ({“et”},{“130”},1.05));  
  // n_bjets>=1
  
  cb.cp().process({"bbH-NLO"}).AddSyst(cb, "QCDScale_bbH","lnN", SystMap<channel,bin_id,mass>::init 
      ({"mt"}, {8}, {"130"}, 0.97)
      ({"mt"}, {9}, {"130"}, 1.05)
      ({"mt"}, {10}, {"130"}, 0.95)
      ({"mt"}, {11}, {"130"}, 0.69)
