for MODEL in default2 "$1"; do for TOYS in default2 "$1"; do \
  combineTool.py -M CollectLimits output/${MODEL}/mt/higgsCombine.ggH.toys.${TOYS}.* \
  --use-dirs -o mssm_bias_${MODEL}_model_${TOYS}_toys_ggH.json --toys
  combineTool.py -M CollectLimits output/${MODEL}/et/higgsCombine.ggH.toys.${TOYS}.* \
  --use-dirs -o mssm_bias_${MODEL}_model_${TOYS}_toys_ggH.json --toys
done; done
