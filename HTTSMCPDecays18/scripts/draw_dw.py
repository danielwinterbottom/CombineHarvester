...
def data(ax, df, label, bins, variable_bin=False, data_kw={}, diff=False):
    if not variable_bin:
    ...
    if not diff:
      ...
      ax.errorbar(
          bin_cents, df["sum_w"], yerr=[df["sum_ww_down"].values**.5, df["sum_ww_up"].values**.5],
          #bin_cents, df["sum_w"], yerr=[df["sum_ww"].values**.5, df["sum_ww"].values**.5],
          **kwargs,
      )
...
def mc(
    ...
    if 'Bestfit_stack' in order:
      order.remove('Bestfit_stack')
      order.append('Bestfit_stack')
      tdf = tdf[order]
...
def data_mc(
    ...
    ratio_legend_kw={}, mcstat_ratio_kw={}, variable_bin=False, difference=False,#mc_for_ratio=None,
):
    ...
    bestfit_mask = ~_df_mc.index.get_level_values("parent").isin(sigs+['Bestfit_stack']) 

    df_mc_sm = _df_mc.loc[sig_mask].copy(deep=True)
    df_mc_sm_forratio = _df_mc.loc[bestfit_mask].copy(deep=True)
    ...
    df_mc_sum_forratio = df_mc_sm_forratio.groupby(label).sum()
    df_mc_sum_forratio.loc[:,"parent"] = "SMTotal"
    df_mc_sum_forratio = df_mc_sum_forratio.groupby(["parent", label]).sum()
    ...
    if difference:
        ...
        df_mc_sum_ratio = df_mc_sum_forratio.copy()
        df_mc_sum_ratio.loc[:,"sum_w"] = 0.

        df_mc_sum_ratio.loc[:,"sum_ww_up"] = (
            df_mc_sum_forratio["sum_ww_up"]/df_mc_sum_forratio["sum_ww_up"]
        )
        df_mc_sum_ratio.loc[:,"sum_ww_down"] = (
            df_mc_sum_forratio["sum_ww_down"]/df_mc_sum_forratio["sum_ww_down"]
        )
        #set these to 0 as the band is drawn elsewhere
        df_mc_sum_ratio.loc[:,"sum_ww_up"] = 0.
        df_mc_sum_ratio.loc[:,"sum_ww_down"] = 0.
        ...
        if not blind:
            ...
            df_data_ratio.loc[:,"sum_ww_down"] = df_data_ratio.loc[:,"sum_ww_down"]*df_data_ratio.loc[:,"sum_ww_down"].values

            data(ax[1], df_data_ratio, label, bins, variable_bin, data_kw=kwargs, diff=True)
...
def nllscan(
    ...
    for nsig in nsigs:
        ...
        kw.update(line_kw)
        #comment to not draw vertical lines
