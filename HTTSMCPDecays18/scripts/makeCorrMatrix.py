...
for mode,pois in modes.iteritems():
  ...
  fileName = 'output/paper_230121/cmb/125/robustHesse.bestfit.robustHesse.root'
  ...
  for iPar,par in enumerate(theList):
    ...
    #if iPar==len(theList): break
    #if not par.GetName().startswith('r_'): continue
    if par.GetName().startswith('mu'): print par.GetName()
    if par.GetName() not in ['muV','muggH','alpha']: continue
  ...
  latex.DrawLatex(1.00-canv.GetRightMargin()-0.02,1.00-canv.GetTopMargin()-0.06,'#bf{CMS} #it{Supplementary}')
...
#  canv.Print('%s/src/flashggFinalFit/Combine/runFits%s_%s/Plots/corrMatrix_%s_%s%s%s.png'%(os.environ['CMSSW_BASE'],opt.ext,mode,mode,name.split("_")[-1],obs_ext,opt.ext))
  ...
  canv.Print('plots/corrMatrix.pdf')
